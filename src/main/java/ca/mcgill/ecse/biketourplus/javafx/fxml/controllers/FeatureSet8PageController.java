...
public class FeatureSet8PageController {
  ...
  /**
   * This pays the trip for the participant
   * 
   * @param event
   * @author Brian Morava
   */
  ...
  public void payClicked(ActionEvent event) {
    if (emailTextField.getText().equals("")) {
      ...
      String msg = BikeToursFeatureSetController.payForParticipantTrip(emailTextField.getText(),
          authCodeTextField.getText());
      if (msg.equals(
          "Participant with email address " + emailTextField.getText() + " does not exist")) {
        // JOptionPane.showMessageDialog(null, "Participant with email address " +
        // emailTextField.getText() + " does not exist","ERROR",JOptionPane.ERROR_MESSAGE);
        ViewUtils.makePopupWindow("ERROR",
            "Participant with email address " + emailTextField.getText() + " does not exist");
      } else if (msg.equals("Invalid authorization code")) {
        // JOptionPane.showMessageDialog(null, "Please enter an authorization
        // code","ERROR",JOptionPane.ERROR_MESSAGE);
        ViewUtils.makePopupWindow("ERROR", "Please enter an authorization code");
      } else if (msg.equals("")) {
        // JOptionPane.showMessageDialog(null,"Your payment was
        // processed","SUCCESS",JOptionPane.PLAIN_MESSAGE);
  ...
  /**
   * This calculates the trip for the participant and displays it
   * 
   * @param event
   * @author Brian Morava
   */
    ...
    if (id != -1) {
      try {
        ...
        int indexOfParticipant =
            BikeToursFeatureSetController.getParticipantIndex(emailTextField.getText());
        int costCents =
            bikeTourTO.getParticipantCost(indexOfParticipant).getTotalCostForBikingTour();
        totalPriceTextField.setText("$" + Double.toString(costCents));
      } catch (InvalidInputException e) {
        // JOptionPane.showMessageDialog(null, "Invalid participant
        // email","ERROR",JOptionPane.ERROR_MESSAGE);
